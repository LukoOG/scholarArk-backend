...
export class AssessmentsService {
	...
	private async validatePublishable(assessment: Assessment): Promise<void>{
		...
		if(assessment.questions.length === 0) throw new BadRequestException("Assessment must contain at least 1 question");

		if(assessment.questions.length !== 0){
			if(assessment.questions.length < assessment.totalQuestions) throw new BadRequestException("Assessment toal questions must be equal to or less than question bank")
		}
	}
